...
const ContentCard: React.FC<CaseStudyCardProps> = ({ title, category, image, logo }) => {
  return (
    <div
      className="relative flex h-mx-auto sm:w-full h-auto sm:h-[364px] flex-col items-start justify-between rounded-lg p-4"
      style={{
    ...
    >
      {image && <div className="opacity-20 rounded-lg absolute inset-0 bg-black" />}
...
const HoverRevealSlip = ({ show }: { show: React.ReactNode }) => {
  ...
  return (
    <div className={cn("group relative h-56 w-[390px] [perspective:1000px]")}>
      {/* Back cover - static */}
      ...
      <div
        className={cn(
          "relative z-50 h-full w-[370px] origin-left transition-transform duration-500 ease-out [transform-style:preserve-3d] group-hover:[transform:rotateY(-30deg)]",
        )}
      >
      ...
      >
        <div className="-rotate-90 whitespace-nowrap pb-16 pr-9">EN SAVOIR PLUS</div>
      </div>
    </div>
  );
};
